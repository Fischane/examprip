...
import { Plus, Trash2, Download, Edit3, Save, CheckCircle } from 'lucide-react'
...
function EditorContent() {
  ...
  useEffect(() => {
    ...
    fetch(`/api/cv-templates/${templateId}`)
      ...
      .then(t => {
        if (t.defaultContent) {
          setCV(prev => ({ ...prev, ...t.defaultContent }))
        }
        if (t.layoutConfig?.colorScheme) setColorScheme(t.layoutConfig.colorScheme)
        if (t.layoutConfig?.font) setFont(t.layoutConfig.font)
      })
      .catch(() => {})
  }, [templateId])
  ...
  async function handleSave() {
    if (!templateId) { setSaveError('Please select a template first'); return }
    ...
    try {
      ...
      if (res.ok) {
        ...
        router.replace(`/app/cv/editor/${doc._id}`)
      } else {
  ...
  async function handlePrint() {
    // Log download record if we have a saved doc (handled in [id] page)
    ...
    if (!win) return
    win.document.write(buildPrintHtml(cv, fn, ln, scheme, fontCss))
    win.document.close()
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 py-6">
      ...
      {saveError && (
        <div className="mb-4 bg-red-50 border border-red-200 text-red-700 text-xs px-3 py-2 rounded-lg">{saveError}</div>
      )}
      ...
      <div className={`grid gap-6 ${tab === 'edit' ? 'grid-cols-1 xl:grid-cols-2' : 'grid-cols-1'}`}>
          ...
          <div className="space-y-4 text-sm">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                ...
                <div><label className={lbl}>Full Name</label><input value={cv.name} onChange={e => set('name', e.target.value)} className={inp} placeholder="John Doe" /></div>
                <div><label className={lbl}>Job Title</label><input value={cv.title} onChange={e => set('title', e.target.value)} className={inp} placeholder="Software Engineer" /></div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              <div className="grid grid-cols-1 gap-2">
                <div><label className={lbl}>LinkedIn</label><input value={cv.links.linkedin} onChange={e => setLink('linkedin', e.target.value)} className={inp} placeholder="https://linkedin.com/in/..." /></div>
                <div><label className={lbl}>GitHub</label><input value={cv.links.github} onChange={e => setLink('github', e.target.value)} className={inp} placeholder="https://github.com/..." /></div>
                <div><label className={lbl}>Portfolio</label><input value={cv.links.portfolio} onChange={e => setLink('portfolio', e.target.value)} className={inp} placeholder="https://yourportfolio.com" /></div>
              </div>
            </div>
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-4">
              ...
              {cv.references.map((ref, i) => (
                <div key={i} className="mb-3 pb-3 border-b border-gray-100 last:border-0">
                  ...
                  <div className="grid grid-cols-2 gap-2">
                    <div><label className={lbl}>Name</label><input value={ref.name} onChange={e => setRef(i, 'name', e.target.value)} className={inp} /></div>
                    <div><label className={lbl}>Phone</label><input value={ref.phone} onChange={e => setRef(i, 'phone', e.target.value)} className={inp} /></div>
                    <div><label className={lbl}>Address</label><input value={ref.address} onChange={e => setRef(i, 'address', e.target.value)} className={inp} /></div>
                    <div><label className={lbl}>Email</label><input value={ref.email} onChange={e => setRef(i, 'email', e.target.value)} className={inp} /></div>
                  </div>
                </div>
              ))}
            </div>
        ...
        <div className={tab === 'preview' ? 'max-w-3xl mx-auto w-full' : ''}>
          <div ref={printRef} className="flex shadow-lg rounded-xl overflow-hidden min-h-[900px]" style={{ fontFamily: fontCss }}>
            <div className="w-48 flex-shrink-0 text-white p-5 flex flex-col gap-0" style={{ background: scheme.sidebar }}>
...
function buildPrintHtml(cv: CV, fn: string, ln: string, scheme: any, fontCss: string): string {
  return `<!DOCTYPE html><html><head><title>${cv.name || 'CV'}</title>
  ...
  <div style="width:200px;flex-shrink:0;background:${scheme.sidebar};color:#fff;padding:24px 16px;min-height:100vh">
    ...
    ${cv.educations.some(e => e.school) ? `
      <div style="font-size:9px;font-weight:bold;text-transform:uppercase;color:${scheme.accent};border-bottom:1px solid #444;padding-bottom:5px;margin:14px 0 8px">Education</div>
      ${cv.educations.filter(e => e.school).map(e => `<div style="margin-bottom:10px"><div style="font-size:10px;font-weight:bold;color:${scheme.accent}">${e.school}</div><div style="font-size:9px;color:#aaa">${e.degree}</div><div style="font-size:9px;color:#777">${e.from}${e.to ? ` - ${e.to}` : ''}</div></div>`).join('')}
    ` : ''}
  </div>
  <div style="flex:1;background:#fff;padding:32px 28px">
    <div style="margin-bottom:18px">
      <div style="font-size:26px;font-weight:900;color:#222;line-height:1.1">${fn ? `${fn} ` : ''}<span style="color:${scheme.accent}">${ln}</span></div>
    ...
    ${cv.experiences.some(e => e.role) ? `
      <div style="font-size:9px;font-weight:bold;text-transform:uppercase;color:${scheme.accent};border-bottom:2px solid ${scheme.accent};padding-bottom:4px;margin:18px 0 10px">Experience</div>
      ${cv.experiences.filter(e => e.role).map(e => `<div style="margin-bottom:12px;padding-left:10px;border-left:2px solid ${scheme.accent}"><div style="display:flex;justify-content:space-between"><div style="font-size:11px;font-weight:bold;color:#222;text-transform:uppercase">${e.role}</div><div style="font-size:9px;color:#999">${e.from}${e.to ? ` - ${e.to}` : ''}</div></div>${e.company ? `<div style="font-size:10px;color:#888;font-style:italic">${e.company}</div>` : ''}${e.desc ? `<div style="font-size:10px;color:#555;margin-top:3px;line-height:1.5">${e.desc}</div>` : ''}</div>`).join('')}
    ` : ''}
    ${cv.skills.some(s => s.name) ? `
      <div style="font-size:9px;font-weight:bold;text-transform:uppercase;color:${scheme.accent};border-bottom:2px solid ${scheme.accent};padding-bottom:4px;margin:18px 0 10px">Skills</div>
      <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px 16px">
        ${cv.skills.filter(s => s.name).map(s => `<div><div style="font-size:10px;color:#333;margin-bottom:3px">${s.name}</div><div style="height:5px;background:#eee;border-radius:3px"><div style="height:5px;background:${scheme.accent};border-radius:3px;width:${s.level}%"></div></div></div>`).join('')}
      </div>
    ` : ''}
  </div>
  </body></html>`
...
export default function CVEditorPage() {
  return (
    <Suspense fallback={<div className="flex items-center justify-center py-20 text-gray-400">Loading editor...</div>}>
      <EditorContent />
    </Suspense>
  )
}
